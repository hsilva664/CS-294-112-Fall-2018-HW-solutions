...
class Histogram(Density_Model):
    ...
    def get_count(self, states):
        ...
        counts = np.zeros((bsize), np.float)
...
class RBF(Density_Model):
    ...
    def fit_data(self, data):
        ...
        self.means = np.stack(data, axis=0)
    ...
    def get_prob(self, states):
        ...
        if self.means is None:
            ...
            # 1. Compute deltas
            temp1 = np.expand_dims(self.means, axis = 0)
            temp2 = np.expand_dims(states, axis = 1)
            deltas = temp2 - temp1
...
class Exemplar(Density_Model):
    ...
    def build_computation_graph(self):
        ...
        self.update_op = tf.train.AdamOptimizer(self.learning_rate).minimize(-1*self.elbo)
    ...
    def make_prior(self, z_size):
        ...
        """
        prior_mean = tf.zeros(z_size, dtype = tf.float32)
        prior_logstd = tf.zeros(z_size, dtype = tf.float32)
    ...
    def forward_pass(self, state1, state2):
        ...
        assert z1.shape[1] == self.hid_dim/2 and len(z1.shape) == 2
        assert z.shape[1] == self.hid_dim and len(z.shape) == 2
    ...
    def get_likelihood(self, state1, state2):
        ...
        likelihood = self.sess.run(self.likelihood, feed_dict = {self.state1: state1, self.state2: state2, self.discrim_target: np.expand_dims(np.logical_and.reduce(state1 == state2, 1).astype(float), axis=1) })
        return likelihood
